...
- Processes are added with `add_process()` for the sole purpose of simulating a part of the netlist with behavioral Python code.
  - Typically such a process will consist of a top-level `async for values in sim.tick().samples(...):` or `async for values in sim.changed(...)`, but this is not a requirement.
...
- `until(condition)`
  - Repeat the trigger until `condition` is true.
    ...
    It is only awaitable once and returns the value(s) of the sampled expression(s) at the last time the trigger was repeated, or `None` if the simulation was not advanced.
  - Example implementation:
    ```python
    async def until(self, condition):
        values, done = None, await self._sim.get(condition)
        while not done:
